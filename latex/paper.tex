...
\usepackage{tabularx}
\usepackage{float}
\usepackage{makecell} % Used for multi-line column headers
\renewcommand\theadalign{bc} % Centers header text horizontally and vertically
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\columnwidth]{figures/fig_latency_comparison.pdf}
    \caption{Query Latency vs. Recall. ZGQ achieves lower latency for equivalent recall levels compared to HNSW. The gap widens at higher recall targets, demonstrating ZGQ's efficiency in navigating the graph to find hard-to-reach neighbors.}
