...
The complete source code, benchmarking scripts, and mathematical proof documents supporting this study are available in the GitHub repository at \url{https://github.com/nathangtg/dbms-research}. The repository contains the implementation of the ZGQ framework (Version 8) and the LaTeX source for this manuscript. Note that the large-scale synthetic datasets used in the experiments are not directly included in the repository due to size constraints; however, the repository provides the necessary scripts with deterministic seeds to fully reproduce the datasets and experimental results.
