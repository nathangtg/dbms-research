...
NSW algorithm degrades with clustered data and has polylogarithmic complexity. & 
HNSW: Builds multi-layer graph with exponentially decaying layer probability, separating links by distance scale. Search starts from top layer and descends greedily. & 
Achieves $O(\log N)$ complexity, outperforming NSW on clustered data. \\
\midrule

Wang et al. \cite{wang2021} & 
Lack of systematic comparison for diverse graph-based ANNS algorithms. & 
Survey and comparison of 13 graph algorithms. Taxonomy based on base graphs (DG, RNG, KNNG, MST) with fine-grained component analysis. & 
Performance is highly dataset-dependent. Increased graph quality does not guarantee better search. \\
\midrule

Prokhorenkova \& Shekhovtsov \cite{prokhorenkova2020} & 
Lack of theoretical guarantees for graph-based ANNS, especially in low dimensions ($d \ll \log n$). & 
Theoretical analysis of greedy graph search. Investigated long-range links and Beam Search. & 
Long-range links reduce steps to $O(\log^2 N)$. Beam Search improves accuracy and enables sparser graphs. \\
\midrule

Chen et al. \cite{chen2021} & 
High memory cost of in-memory ANNS (HNSW) for billion-scale datasets. & 
SPANN: Hybrid approach storing centroids in memory, full vectors on SSD. Uses balanced hierarchical clustering. & 
Outperforms DiskANN on billion-scale data, achieving $>90\%$ recall with lower latency. \\
\midrule

Manohar et al. \cite{manohar2024} & 
Scalability limitations of parallel graph-based ANNS due to locks and sequential bottlenecks. & 
ParlayANN: Deterministic parallel graph-based ANNS library using prefix doubling for batch insertions. & 
Efficient scalable construction on multi-core processors, outperforming HNSW and DiskANN. \\
\midrule

Akhil \& Sivashankar \cite{akhil2025} & 
HNSW performance degradation on billion-scale datasets. & 
ZHNSW: Partitions dataset into zones via K-Means. Each zone maintains independent HNSW graph. & 
Improves HNSW scalability but introduces memory overhead from multiple graphs. \\
\midrule

Zhang et al. \cite{zhang2025} & 
I/O latency bottleneck in disk-based ANNS with SSD-resident indices. & 
XN-Graph with extended neighborhood coverage; BBP for construction; IMF Search prioritizing cached nodes. & 
Achieves $>4000$ QPS and $>0.9$ recall on billion-scale disk search with fewer hops and optimized I/O. \\
\midrule

Gollapudi et al. \cite{gollapudi2023} & 
Poor performance when queries include metadata filters, especially with low specificity. & 
FilteredVamana, StitchedVamana: Label-aware graph construction. Edges based on proximity and shared labels. & 
$>10\times$ speedup over post-processing methods, achieving high throughput and recall on SSDs. \\
\midrule

Cai et al. \cite{cai2024} & 
Lack of versatility in filtered ANNS to handle diverse label constraints efficiently. & 
UNG: Framework combining vector proximity graph with LNG (DAG encoding label relationships). & 
Guarantees versatility, fidelity, completeness, and adaptability for filtered search. \\
\midrule

Xu et al. \cite{xu2023} & 
Difficulty maintaining performance and avoiding costly rebuilds under incremental updates. & 
SPFRESH: SPANN extension with LIRE protocol for local partition rebalancing after splits. & 
Enables efficient in-place updates for billion-scale indices without full reconstruction. \\
\midrule

Gong et al. \cite{gong2025} & 
High latency and inefficiency when using detached vector DBs with streaming systems. & 
VStream: Integrated streaming vector search engine with dynamic LSH-based partitioning. & 
Achieves $373\times$ faster throughput with lower CPU and memory consumption. \\
\midrule

Coleman et al. \cite{coleman2022} & 
Inefficient memory access in graph-based ANNS leads to poor CPU cache utilization. & 
Graph Reordering: Reorganizes node memory layout to place co-accessed nodes physically closer. & 
Up to 40\% faster query latency through improved cache hit rates. \\
\midrule

Gao \& Long \cite{gao2023} & 
DCOs dominate runtime, performing full $O(D)$ computation even for discarded candidates. & 
ADSampling: Randomized DCO with offline transformation and online incremental dimension sampling. & 
Reduces DCO complexity from $O(D)$ to $O(\log D)$ with negligible accuracy loss. \\
\midrule

Chen et al. \cite{chen2023} & 
Many distance computations during search don't influence final results. & 
FINGER: Approximates distances using vector decomposition relative to center node and LSH-based angle estimation. & 
Speeds up HNSW by 20\%--60\% without significantly impacting accuracy. \\
\midrule

Peng et al. \cite{peng2023} & 
Lack of practical theoretical guarantees; existing ones rely on impractical assumptions. & 
$\tau$-MG: Guarantees finding 1-NN if $\delta(q,\bar{p}_1) < \tau$ with $O(n^{1/d}(\ln n)^2)$ complexity. Proposes $\tau$-MNG variant. & 
Provides theoretical guarantee with better complexity. $\tau$-MNG shows strong empirical performance. \\
\midrule

Xie et al. \cite{xie2025} & 
No guarantees for $k$-NN ($k>1$) search; requires impractical predefined global $\tau$. & 
LMG: Edges labeled with minimum $\tau$. Two-phase: Adaptive Search (1-NN) + Refinement Search ($k-1$ neighbors). & 
First approach guaranteeing exact $k$-NN for arbitrary queries without predefined $\tau$. \\
\midrule

Vecchiato et al. \cite{vecchiato2024} & 
Routing bottleneck in cluster-based ANNS due to simple heuristics. & 
Reformulates cluster selection as Learning to Rank problem. Trains function to rank clusters. & 
Consistently improves Top-1 accuracy and search performance through learned routing. \\
\midrule

Yang et al. \cite{yang2025} & 
Super-linear construction time for SOTA graph ANNS limits scalability. & 
Identifies $k$-CNA as bottleneck. Proposes RNG-based pruning before $k$-CNA and global layering for HNSW. & 
Accelerates construction up to $5.6\times$ (NSW) and $4.6\times$ (HNSW) without degrading search. \\
\midrule

Zhao et al. \cite{zhao2020} & 
Difficulty adapting iterative, dependency-heavy graph ANNS for GPU parallelization. & 
SONG: Decouples search into parallelizable stages (Seed Selection, Expansion, Merging) with GPU optimizations. & 
Achieves $50$--$180\times$ speedup over single-thread CPU HNSW, outperforms Faiss GPU. \\
\midrule

Li et al. \cite{(li2023)} & 
Lack of comprehensive survey of deep learning applications in ANNS. & 
Categorizes DL-based ANNS into "Learning to Index" and "Learning to Search". Proposes end-to-end learning. & 
Provides systematic overview of DL in ANNS. Highlights benefits of end-to-end optimization. \\
...
% (This comes just before your references)
% ----------------------------------------------------------------
