...
A core contribution of this work lies in the rigorous mathematical derivation of the framework's performance bounds. We established the \textbf{Improved Local Connectivity} factor, denoted as $\gamma$, by analyzing the probability of edge formation during graph construction. In a standard HNSW, random insertion yields a low probability ($P_{same} \approx n_z/n$) that a node connects to a semantically similar neighbor within the same cluster. Conversely, ZGQ's zone-ordered insertion strategy ensures that vectors from the same zone are processed consecutively, raising this probability towards unity ($P_{same} \rightarrow 1$). By quantifying the ratio of expected intra-zone edges between the two methods, we derived the equation $\gamma = 1 + \frac{\alpha \cdot (M_{ZGQ} - M_{HNSW})}{M_{HNSW}}$. With an empirical intra-zone density factor $\alpha \approx 0.65$ and increased local connectivity $M_{ZGQ}=32$, we proved a theoretical connectivity improvement of $\gamma \approx 1.65$.

Furthermore, we derived the \textbf{Search Path Length Reduction} by decomposing the search complexity into inter-zone and intra-zone components. We modeled the search process not as a single greedy descent, but as a two-stage traversal: a rapid inter-zone navigation ($\frac{\log n}{Z}$) followed by a bounded intra-zone search. By introducing a zone-guidance efficiency factor $\beta$, which represents the "highway effect" of traversing densely connected local subgraphs, we arrived at the reduction formula $\mathbb{E}[L_{ZGQ}] = \mathbb{E}[L_{HNSW}] \cdot (1 - \frac{\beta}{Z})$. For a sufficiently large number of zones $Z$, this simplifies to a path reduction of $(1 - \beta)$, theoretically predicting the 22\% latency reduction observed in our benchmarks. Finally, our space complexity analysis confirmed that the overhead of maintaining zone centroids scales as $\mathcal{O}(\sqrt{N})$, ensuring that ZGQ maintains the asymptotic space efficiency of HNSW while delivering superior query performance.
